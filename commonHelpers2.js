import{l as r,c as nt,u as w,e as C,a as f,s as p,f as v,g as L,d as $,h as P,i as ct,j as it,k as _,m as rt,n as lt,o as dt}from"./assets/scroll-up-855fb9c5.js";import"./assets/vendor-9038a9ee.js";const A="basket",ut=document.querySelector(".product-list"),pt=r(A)??[];async function mt(t){const e=r(A)??[];if(!t.target.closest(".btn-basket"))return;const a=t.target.closest(".resp-item").dataset.id,c=r("main products").filter(({_id:i})=>a===i)[0],n=document.querySelectorAll(`[data-button-id="${a}"]`);w(A,c,e),C([...n],a,e)}nt(pt);ut.addEventListener("click",mt);const ft=document.querySelector(".product-list"),gt="main products";function x(t){f(gt,t);const e=r("basket")??[],s=t.map(a=>{const o=yt(a.category),c=bt(a.price),n=e.find(({_id:i})=>i===a._id);return`<li class="resp-item" data-id="${a._id}">
      ${a.is10PercentOff?`<svg class="svg-discount animation-icon" width="60" height="60">
        ...
        aria-label="This is 1 page">
        1
      </button>`,s+='<span class="dots">...</span>'),e===l?(a=a-1,H.classList.add("disabled")):e===l-1&&(a=a-1),e===1?(o=o+1,F.classList.add("disabled")):e===2&&(o=o+1);for(let n=a;n<=o;n+=1)if(!(n>l||n<0)){if(n===0&&(n+=1),c>=768){if(l===4){if(e===3){if(n===1)continue}else if(e===2&&n===4)continue}else if(l===3){if(e===3){if(n===1)continue}else if(e===1&&n===3)continue}}s+=`<button 
      type="button" 
      ...
      data-page="${l}">
      ${l}
    </button>`),kt.innerHTML=s,s}function $t(t){const e=t.target.closest("button");if(!e)return;const s=e.dataset.page;s==="next-page"?g+=1:s==="prev-page"?g-=1:g=Number(s),K!==g&&(Pt(g),I(l,g),K=g)}async function Pt(t){const s={...r(ht),page:t};try{v();const{results:a}=await L(s);x(a)}catch{vt.classList.remove("visually-hidden"),W.classList.add("visually-hidden"),Lt.innerHTML="",$()}finally{P()}}W.addEventListener("click",$t);const St={popularList:document.querySelector(".popular-list")},wt=r("basket")??[];function Ct(t){const e=t.map(({name:s,img:a,category:o,size:c,popularity:n,_id:i})=>{const u=wt.some(q=>q._id===i);return`<li class="popular-item" data-id="${i}">
                ...
                </li>`});St.popularList.insertAdjacentHTML("beforeend",e.join(""))}const qt=5;ct(qt).then(t=>{Ct(t),f("popular products",t)}).catch(t=>console.log(t));const At={popularList:document.querySelector(".popular-list")};async function Et(t){const e="basket",s=r(e)??[];if(!t.target.closest(".btn-light-basket"))return;const o=t.target.closest(".popular-item").dataset.id,n=r("popular products").filter(({_id:u})=>o===u)[0],i=document.querySelectorAll(`[data-button-id="${o}"]`);w(e,n,s),C([...i],o,s)}At.popularList.addEventListener("click",Et);const Bt={discountList:document.querySelector(".discount-list")},Tt=r("basket")??[];function Mt(t){const e=t.map(({name:s,img:a,_id:o,price:c})=>{const n=Tt.some(i=>i._id===o);return`<li class="discount-item" data-id="${o}">
    ...
    <div class="desc-text"> <p class="textModalp">${i.desc||"Description will be added soon"}</p></div>
    </div>
    </div>
    <div class="footer-product_card2">
    <p class="price-product">$${i.price.toFixed(2)}</p></div>
    <div class = "end-modal">
    <button 
      type="button" 
      class="modal-button" 
    ...
    <use class="href-icon" href="${p}#icon-basket"></use>
  </svg></button></div>`;Qt.innerHTML=q,M.style.display="block",document.body.style.overflow="hidden",j(),document.querySelector(".modal-button").addEventListener("click",et)}function se(t){const e=r("main products"),s=r("popular products"),a=r("discount products");return[...e,...s,...a].find(n=>n._id===t)}function ae(t,e){const s=se(t),a=document.querySelectorAll(`[data-button-id="${t}"]`);w(tt,s,e),C([...a],t,e);const o=e.find(n=>n._id===t),c=document.querySelector(".textInButton");o?c.textContent="Remove from":c.textContent="Add to"}Xt.addEventListener("click",D);te.addEventListener("click",D);ee.addEventListener("click",D);
